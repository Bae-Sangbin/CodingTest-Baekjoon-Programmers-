SELECT AU.AUTHOR_ID, AU.AUTHOR_NAME, ID.CATEGORY, ID.TOTAL_SALES AS TOTAL_SALES
...
# SELECT AU.AUTHOR_ID, AU.AUTHOR_NAME, ID.CATEGORY, ROUND(ID.TOTAL_SALES/CNT,0) AS TOTAL_SALES
...
# (SELECT B.AUTHOR_ID, B.CATEGORY, SUM(BS.SALES) * SUM(B.PRICE) AS TOTAL_SALES, COUNT(*) AS CNT
# FROM BOOK B JOIN BOOK_SALES BS ON B.BOOK_ID = BS.BOOK_ID
# WHERE BS.SALES_DATE LIKE '2022-01%'
# GROUP BY AUTHOR_ID, CATEGORY) ID ON AU.AUTHOR_ID = ID.AUTHOR_ID
# ORDER BY ID.AUTHOR_ID, ID.CATEGORY DESC
